...
//
// 入力:  out/01_osm_raw.json
// 出力:  out/02_spots_with_pref.json … 都道府県コード・市区町村名を付与したスポット一覧
...
const main = async () => {
  const raw = await readJson(path.join(OUT_DIR, '01_osm_raw.json'));
  const muniCodes = await readJson(path.join(SPOTS_DIR, 'muni_codes.json'));
  const cache = (await readJsonIfExists(cachePath)) ?? {};

  const spots = (raw.elements ?? []).map(extractSpot).filter(Boolean);
